...
public class SeekBarPreference extends DialogPreference implements SeekBar.OnSeekBarChangeListener, OnClickListener {
	...
	public interface OnSeekBarPreferenceDoneListener {
		boolean onPreferenceDone(Preference preference, Object newValue);
	}
	...
	protected boolean callDoneListener(Object objValue) {
	...
	OnSeekBarPreferenceChangeListener mOnChangeListener = null;
    
    public void setOnPreferenceChangeListener(OnSeekBarPreferenceChangeListener onPreferenceChangeListener) {
    	mOnChangeListener = onPreferenceChangeListener;
    }
	protected boolean callChangeListener(Object objValue) {
		return mOnChangeListener == null ? true : mOnChangeListener.onPreferenceDone(this, objValue);
	}
	*/
	...
	public void setValue(int max, int min, String value) {
		mMax = max;
		mMin = min;
		if (mStep == 0) {
			mStep = 1;
	...
	public void setValue(int max, int min, int step, String value) {
		mMax = max;
		mMin = min;
		mStep = step;
		mValue = value;
    ...
    protected void onDialogClosed(boolean positiveResult) {
        if (positiveResult) {
        	...
        	if (callDoneListener(mValue)) {
				SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(this.getContext());
				Editor ed = settings.edit();
				ed.putString(key, mValue);
				ed.commit();
        	}
        }
    }
	...
	public void onProgressChanged(SeekBar seekBar, int progress,
			boolean fromUser) {
        mValue = Integer.toString(progress + mMin);
        mTextView.setText(mValue);
